...
st.set_page_config(
    page_title="CyberShield SIH 2026",
    page_icon="C",
    layout="wide",
)
...
st.title("CyberShield - Detection, Prioritization & Simulated Response")
st.caption(
    "SIH 2026 MVP | Controlled cybersecurity lab | "
    "All response and remediation actions are simulated."
)
...
if st.sidebar.button("Simulate Attack Scenario"):
    ...
    st.session_state.events.extend(
        [
            event
            for event in attack_scenario_events()
...
if uploaded_file is not None and st.sidebar.button("Load Uploaded Events"):
    ...
    st.sidebar.success(
        f"Loaded {len(new_events)} controlled demo events."
    )
